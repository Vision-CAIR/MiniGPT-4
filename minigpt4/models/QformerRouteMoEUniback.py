...
class UniFeedForward(nn.Module):
    def __init__(self, config):
...
class BertLayer(nn.Module): 
    def __init__(self, config, layer_num):
        ...
        self.universal_expert = UniFeedForward(config)        
        ...
        if query_length > 0:
            ...
            if attention_output.shape[1] > query_length: # have text input in Qformer
                ...
                if self.layer_judge == 'first' and self.num_beams>1:
                    bz = query_attention_output.shape[0]
                    moe_ffn_attention_input = query_attention_output[:, :query_length, :] # [bz, 32, hidden_dim]
                    moe_ffn_attention_mask = attention_mask.squeeze(dim=1).squeeze(dim=1)[:, :query_length] 
                    layer_output = self.feed_forward_query_moe(moe_ffn_attention_input, moe_ffn_attention_mask, beam_scores, expert_route)  # [bz*num_beam, 32, hidden_dim]
                    expert_output_hidden = layer_output[0]
                    universal_output = self.universal_expert(moe_ffn_attention_input)# [bz, 32, hidden_dim]
                    lst = list()
                    for i in range(bz):
                        lst.append(universal_output[i,:,:])
                        lst.append(expert_output_hidden[i*self.num_beams:(i+1)*self.num_beams,:,:])
                    uni_exp_output = torch.stack(lst) # [bz*(num_beam+1), 32, 768]
                    layer_output_text = self.adjust_hidden_states_by_num_beams(layer_output_text, add_universal=True) #[bz*(num_beam+1), len, hidden_dim]
                    layer_output = (torch.cat([uni_exp_output, layer_output_text], dim=1), layer_output[1], layer_output[2], layer_output[3],layer_output[4])
                    # layer_output = (hidden_states, beam_scores, expert_route, beam_idx, importance_loss)

                if self.layer_judge == 'mid' and self.num_beams > 1:
                    # layer_output_text [bz*num_beams, len, hidden_size]
                    bz = query_attention_output.shape[0] / (self.num_beams+1)
    ...
    def adjust_hidden_states_by_num_beams(self, hidden_states, add_universal=False):
        batch_size, text_length, hidden_size = hidden_states.shape
        if add_universal:
            tmp_text = hidden_states.unsqueeze(1).expand(batch_size, self.num_beams+1, text_length, hidden_size)
        else:
    ...
    def feed_forward_query_moe(self, attention_output, expert_attention_mask, beam_scores, expert_route):
        ...
        experts_output = self.expert_ln(hidden_states + attention_output) # [bz*num_beams, 32, hiddem_dim]

        universal_output = self.universal_expert(input_tensor)
...
class BertMoERouteLMHeadModelUniverse(BertPreTrainedModel):
